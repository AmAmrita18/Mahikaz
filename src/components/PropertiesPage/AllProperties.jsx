    ...
    >
      <div className="w-full h-full bg-[#937D6480]">
        ...
        <div className="w-[80%] h-full mx-auto max-w-[1200px] py-28">
          ...
          <h2 className="text-[16px] text-center  text-black leading-[24px] ">
            124 results
          </h2>
          <DropdownList />
          <div className="grid lg:grid-cols-3  md:grid-cols-2 grid-cols-1 gap-x-12 gap-y-8">
            {estates.map((est) => (
              <Link to={`/property/${est.id}`} key={est.id}>
                <div
                  key={est.id}
                  className="flex flex-col shadow-2xl  transition-all duration-700 ease-in-out  hover:scale-95  bg-[#fff] rounded-[32px]"
                >
                  <img
                    src={est.images[0]}
                    alt="thumbnail"
